...
base_path = '/nobackup/scdrw/plume/unstratified/pr_05/';

% Find relevant directories.
locations = {};
counter = 1;
dirs = dir(base_path); dirFlags = [dirs.isdir]; dirs = dirs(dirFlags);
for i = 1:length(dirs)
...
for Li = 1:nlocations
    ...
    path_to_files = [ base_path, locations{Li}, '/azimuthal_averages/' ];
    ...
    if nfiles > 0
        ...
        if flag == 1
            ...
            x_duplicates = A.data(idx:end,1);
            z_duplicates = A.data(idx:end,2);
...
for Li = 1:nlocations
    ...
    for ti = 1:jump:nfiles
        ...
        file_name = [base_path,path_to_files,'azs2d.fld',num2str(ti,'%02.f')];
        A = importdata(file_name,' ');
        ...
        % Read field data.
        %u_all = A.data(:,4);
        w_all = A.data(idx:end,5);
        T_all = A.data(idx:end,6);
